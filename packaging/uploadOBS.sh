   ...
   venvSnapshot="venv-al9-${venvVersion}.tar.gz"
        version="2025.07.21"
         obsDir="../coriolis-obs/home:jpc-lip6/coriolis-pdk-gf180mcu"
 ...
 if [ "${doSources}" = "true" ]; then
   ...
   ./packaging/git-archive-all.sh -v --prefix coriolis-pdk-gf180mcu-${version}/ \
                                     --format tar.gz \
                                     coriolis-pdk-gf180mcu-${version}.tar.gz
 fi
 ...
 for distribFile in packaging/coriolis-pdk-gf180mcu.spec      \
                    packaging/coriolis-pdk-gf180mcu-rpmlintrc \
                    packaging/patchvenv.sh                     \
                    ${venvSnapshot}                            \
                    coriolis-pdk-gf180mcu-${version}.tar.gz   \
                    packaging/coriolis-pdk-gf180mcu.dsc       \
                    packaging/coriolis-pdk-gf180mcu-rpmlintrc \
 ...
 sed -i "s,^Release: *1,Release:        <CI_CNT>.<B_CNT>.${githash}," ${obsDir}/coriolis-pdk-gf180mcu.spec
